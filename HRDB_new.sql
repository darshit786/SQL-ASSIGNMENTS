...
select department_id, department_name, count(employee_id) from emp_details_view group by department_name;
...
-- 6) Display the employees name, department id and commission. If an Employee doesn't earn the commission, then display as 'No commission'. Name the columns appropriately
 select first_name, last_name, department_id, commission_pct as no_commission from employees where commission_pct is null;
...
-- 10) Select the Managers name, the count of employees working under and the department ID of the manager.
 select department_id, manager_id,count(employee_id) from employees  group by department_id;
...
--  11) Select the employee name, department id, and the salary. Group the result with the manager name and the employee last name should have second letter 'a!
 select department_id, salary, concat(first_name," ", last_name)as fullname, manager_id from employees  where last_name like "%_a%"  group by manager_id;
...
select department_id, avg(sum_of_salaries) as average_salary
from (
    select department_id, sum(salary) as sum_of_salaries
    from employees
...
select salary, department_id from employees where salary= (select max(salary)from employees) group by department_id; 
...
select e.department_id, e.last_name, d.department_name from employees e
join departments d 
on e.department_id=d.department_id
